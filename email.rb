...
ip = SETYOURIPHERE
port = 25
domain = TARGETDOMAINHERE
user_base = "user"
number_of_users = 246

source_domain = SOURCEDOMAINHERE
...
emails_to_send.times do
  user_id = rand(number_of_users)+4
  source_user_id = rand(number_of_users)+4
  source_user = source_user_base.to_s + source_user_id.to_s + "@" + source_domain.to_s
  user = user_base.to_s + user_id.to_s + "@" + domain.to_s
  email_lines = (-1*Math.log(1.0-rand(1000000)/1000000.0)*1000+1).truncate+300
  email_words_per_line = (-1*Math.log(1.0-rand(1000000)/1000000.0)*10+1).truncate+1

  puts("MAIL FROM: #{source_user} RCPT TO: #{user} Lines: #{email_lines} Words: #{email_words_per_line}")
